...
type TabType = 'playlist' | 'history' | 'bookmarks';

export function Sidebar({ isOpen, onClose }: SidebarProps) {
  const { playlist, history, bookmarks, loadMedia, removeFromPlaylist, removeBookmark, clearPlaylist, clearHistory, clearBookmarks, addBookmark, showToast } = usePlayer();
              ...
              <div className="flex-1 overflow-y-auto">
                {items.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-full text-slate-500 p-4">
                    <tab.icon className="w-12 h-12 mb-3 opacity-50" />
                    <p className="text-sm">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            <span
                              ...
                              style={{
                                backgroundColor: `${getMediaTypeColor(item.type)}20`,
        ...
        {contextMenu && (
          <>
            ...
            >
              <button
                onClick={() => {
                  loadMedia(contextMenu.item.url, contextMenu.item.title);
                  closeContextMenu();
                }}
              ...
              <button
                onClick={() => handleCopyUrl(contextMenu.item.url)}
                className="w-full px-4 py-2.5 flex items-center gap-3 hover:bg-white/5 text-sm"
              >
              ...
              <button
                onClick={() => handleOpenInNewTab(contextMenu.item.url)}
                className="w-full px-4 py-2.5 flex items-center gap-3 hover:bg-white/5 text-sm"
              >
              ...
              {activeTab !== 'bookmarks' && (
                <button
                  onClick={() => handleAddBookmark(contextMenu.item)}
                  className="w-full px-4 py-2.5 flex items-center gap-3 hover:bg-white/5 text-sm"
                >
